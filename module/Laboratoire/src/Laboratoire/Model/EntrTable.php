...
class EntrTable {
    ...
    public function findAll($start = null, $length = null, $search = null, $order = null, $whr = null) {
        ...
        if ($search != null) {
            ...
            foreach ($search as $srch) {
                if ($srch != "") {
                    if ($i == 0) {
                        $where->addPredicate(new \Zend\Db\Sql\Predicate\Like('ArriHoro', "%$srch%"))
                                ...
                                ->orPredicate(new \Zend\Db\Sql\Predicate\Like('Prec', "%$srch%"));
                        $i++;
                    } else {
                        $where->orPredicate(new \Zend\Db\Sql\Predicate\Like('ArriHoro', "%$srch%"))
                                ->orPredicate(new \Zend\Db\Sql\Predicate\Like('Cta_Nume', "%$srch%"))
                                ->orPredicate(new \Zend\Db\Sql\Predicate\Like('Sexe', "%$srch%"))
                                ->orPredicate(new \Zend\Db\Sql\Predicate\Like('Age_', "%$srch%"))
                                ->orPredicate(new \Zend\Db\Sql\Predicate\Like('Moti', "%$srch%"))
    ...
    public function saveEntr(Entr $entr) {
        ...
        $data = array(
            ...
            'Bi15' => $entr->Bi15,
            'Bioc' => $entr->Bioc,
			'BiocAu01' => $entr->BiocAu01,
			'BiocAu02' => $entr->BiocAu02,
			'BiocAu03' => $entr->BiocAu03,
			'BiocAu04' => $entr->BiocAu04,
			'BiocAu05' => $entr->BiocAu05,
            'Cd4_' => $entr->Cd4_,
