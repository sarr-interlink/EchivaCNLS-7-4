...
class PsyconsconcTable {
    ...
    public function getPsyconsconc($Nume) {
        $Nume =  $Nume;
        $rowset = $this->tableGateway->select(array('Nume' => $Nume));
        $row = $rowset->current();
        if (!$row) {
...
public function getPsyconsconctab() {
        $resultSet = $this->tableGateway->select()->toArray();        
        return $resultSet;
    }
    
    public function savePsyconsconc(Psyconsconc $chrg) {
		$requete = "";
        $data = array(
            'Util' => $chrg->Util,
            'Modi' => $chrg->Modi,
	...
	public function deletePsyconsconc($Nume) {
		$requete = "";
