...
class SociconsconcTable {
    ...
    public function getSociconsconc($Nume) {
        $Nume =  $Nume;
        $rowset = $this->tableGateway->select(array('Nume' => $Nume));
        $row = $rowset->current();
        if (!$row) {
     ...
     public function getSociconsconctab() {
        $resultSet = $this->tableGateway->select()->toArray();        
        return $resultSet;
    }

    public function saveSociconsconc(Sociconsconc $chrg) {
		$requete = "";
        $data = array(
            'Util' => $chrg->Util,
            'Modi' => $chrg->Modi,
	...
	public function deleteSociconsconc($Nume) {
		$requete = "";
