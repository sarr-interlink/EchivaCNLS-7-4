...
class TransTable {
    ...
    public function saveTrans(Trans $trans) {
        ...
        $data = array(
            ...
            'Dat_' => $trans->Dat_,
            'Motif' => $trans->Motif,
            'Structure_source' => $trans->Structure_source,
            'Structure_destination' => $trans->Structure_destination,
        );

        $Nume =  $trans->Nume;
        if(!$this->checkrecord($Nume)){
            $data['Nume']=$trans->Nume;
    ...
    public function deleteTrans($Nume) {
		$requete = "";
