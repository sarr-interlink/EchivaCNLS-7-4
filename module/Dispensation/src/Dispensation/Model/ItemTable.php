...
class ItemTable {
    ...
    public function findAll($start = null, $length = null, $search = null, $order = null,$whr = null,$table = null, $condjoin = null,$columns = null) {
        $select = new \Zend\Db\Sql\Select($this->tableGateway->getTable());
        $selectcount = new \Zend\Db\Sql\Select($this->tableGateway->getTable());
        $selectcount->columns(array('count' => new \Zend\Db\Sql\Expression('COUNT(*)')));
        if ($start != null) {
        ...
        if ($search != null) {
            ...
            foreach ($search as $srch) {
                if ($srch != "") {
                    if ($i == 0) {
                         $where->addPredicate->expression("CONCAT_WS(' ',Dci_Desi,DosaDesi,GaleDesi) LIKE ?",  "%$srch%");
                       // $where->addPredicate(new \Zend\Db\Sql\Predicate\Like('Lot_', "%$srch%"));
                        //Nume dossier à gérer
                        $i++;
                    } else {
                        $where->orPredicate->expression("CONCAT_WS(' ',Dci_Desi,DosaDesi,GaleDesi) LIKE ?",  "%$srch%");
                       // $where->orPredicate(new \Zend\Db\Sql\Predicate\Like('Lot_', "%$srch%"));
                        //Nume dossier à gérer
                    }
                }
        ...
        /*test A Reprogrammer */
        if($table != null && $condjoin){
        $selectcount->join($table,$condjoin,array());
        $select->join($table,$condjoin,array());
        }
        if($columns != null)
        $select->columns($columns,false);
       // echo $selectcount->getSqlString($this->tableGateway->getadapter()->getPlatform());
        //echo $select->getSqlString($this->tableGateway->getadapter()->getPlatform());
       // exit;
        $count = $this->tableGateway->selectWith($selectcount)->current();
        $resultSet = $this->tableGateway->selectWith($select);
        $resultSet->setArrayObjectPrototype(new ItemJoin());
        return array('data' => $resultSet->toArray(), 'count' => $count->count);
    }
    ...
    public function select($where = null, $order = null, $limit = null, $group=null) {
        $select = new \Zend\Db\Sql\Select($this->tableGateway->getTable());
        if ($where != null) {
            $select->where($where);
        }
